...
function DesignCanvas({ children, minScale, maxScale, style }) {
  ...
  const navRef  = React.useRef({ sectionMeta: {}, sectionOrder: [] });
  ...
  navRef.current = { sectionMeta, sectionOrder };
  ...
  const api = React.useMemo(() => ({
    ...
    goTo: (sid, push) => goToRef.current && goToRef.current(sid, push),
    get sectionMeta()  { return navRef.current.sectionMeta; },
    get sectionOrder() { return navRef.current.sectionOrder; },
  }), [state]);
  ...
  return (
    <DCCtx.Provider value={api}>
      <DCViewport minScale={minScale} maxScale={maxScale} style={style} goToRef={goToRef}>{ready && children}</DCViewport>
...
// ─────────────────────────────────────────────────────────────
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {}, goToRef }) {
  ...
  // Smooth-pan the canvas so [data-dc-section="${sectionId}"] lands just below
  // the fixed nav bar. Optionally pushes a history entry for URL navigation.
  const goTo = React.useCallback((sectionId, pushHistory = true) => {
    const world = worldRef.current;
    if (!world) return;
    const el = world.querySelector(`[data-dc-section="${sectionId}"]`);
    if (!el) return;
    const { x: tx, y: ty, scale } = tf.current;
    const r = el.getBoundingClientRect();
    const wx = (r.left - tx) / scale;
    const wy = (r.top  - ty) / scale;
    const targetX = 60 - wx * scale;
    const targetY = 60 - wy * scale; // 52px nav + 8px breathing room
    const sx = tf.current.x, sy = tf.current.y;
    const dur = 380, t0 = performance.now();
    const ease = (p) => p < 0.5 ? 2 * p * p : -1 + (4 - 2 * p) * p;
    const tick = (now) => {
      const p = Math.min(1, (now - t0) / dur);
      tf.current.x = sx + (targetX - sx) * ease(p);
      tf.current.y = sy + (targetY - sy) * ease(p);
      apply();
      if (p < 1) requestAnimationFrame(tick);
    };
    requestAnimationFrame(tick);
    if (pushHistory) { try { history.pushState(null, '', '#' + sectionId); } catch {} }
    window.dispatchEvent(new CustomEvent('dc:section', { detail: sectionId }));
  }, [apply]);

  // Register goTo so DesignCanvas can expose it via context.
  React.useEffect(() => {
    if (goToRef) goToRef.current = goTo;
    return () => { if (goToRef) goToRef.current = null; };
  }, [goTo, goToRef]);

  // Navigate to the hash present at page-load (once sections are rendered).
  React.useEffect(() => {
    const sid = decodeURIComponent(location.hash.slice(1));
    if (!sid) return;
    const t = setTimeout(() => goTo(sid, false), 350);
    return () => clearTimeout(t);
  }, [goTo]);
  ...
  React.useEffect(() => {
    ...
    const onHostMsg = (e) => {
      ...
      if (d && d.type === '__dc_set_zoom' && typeof d.scale === 'number') {
        ...
        apply();
      } else if (d && d.type === '__dc_goto' && d.sectionId) {
        goTo(d.sectionId, d.pushHistory !== false);
      }
    };
...
// ─────────────────────────────────────────────────────────────
// DCNav — fixed top navigation bar
// Reads sections from DCCtx. Each button pans the canvas to that section
// and updates location.hash. Also listens for dc:section events (emitted
// by DCViewport after goTo) and popstate (browser back/forward) to keep
// the active highlight in sync.
// ─────────────────────────────────────────────────────────────
function DCNav({ title = 'Digital Sissy' }) {
  const ctx = React.useContext(DCCtx);
  ...
  React.useEffect(() => {
    ...
    const onPop = () => {
      const sid = decodeURIComponent(location.hash.slice(1));
      setActive(sid || null);
      if (sid && ctx) ctx.goTo(sid, false);
    };
    window.addEventListener('dc:section', onSection);
    window.addEventListener('popstate', onPop);
    return () => {
      window.removeEventListener('dc:section', onSection);
      window.removeEventListener('popstate', onPop);
    };
  }, [ctx]);

  if (!ctx) return null;
  const { sectionOrder, sectionMeta } = ctx;
  if (!sectionOrder || !sectionOrder.length) return null;
  ...
  return (
    <nav style={nav}>
      <span style={{ fontSize: 13, fontWeight: 700, letterSpacing: 0.4,
        color: 'rgba(255,68,68,0.9)', marginRight: 16, flexShrink: 0, whiteSpace: 'nowrap' }}>
        {title}
      </span>
      {sectionOrder.map((sid) => {
        const meta = sectionMeta && sectionMeta[sid];
        if (!meta) return null;
        const on = sid === active;
        // Use only the part before ' · ' as the short label.
        const label = meta.title.split(' · ')[0];
        return (
          <button key={sid}
            title={meta.title}
            onClick={() => { ctx.goTo(sid); setActive(sid); }}
            style={{
            ...
            onMouseEnter={(e) => { if (!on) e.currentTarget.style.color = 'rgba(255,255,255,0.82)'; }}
...
Object.assign(window, { DesignCanvas, DCSection, DCArtboard, DCPostIt, DCNav });
