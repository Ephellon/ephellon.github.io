...
function SynQ(name) {
  ...
  SynQ.set('.uuids', uuids.join(',').replace(/^,/, ''));
...
SynQ.inflate = function(string, mutator) {
  ...
  switch(type) {
    ...
    case 'literal':
    default:
      object = /^"(.*)"$/.test(string)? R.$1: +string <= +Infinity? +string: string === "null"? null: string === "true";
      break;
  }
...
SynQ.help = function(item) {
  ...
  switch(i) {
    ...
    case 'inflate':
      m = "Polyfill of JSON.parse. Inflates\\deserializes strings (turns them into objects)./~Usage: $.@(json-string[, mutator])/~Arguments: String[, Function]/~Returns: Array | Boolean | Object | Number | null | undefined => <object>//string: a JSON compliant string./mutator: mutates each entry. Called on as:/~for(var key in <object>)/~~<mutator>(key, <object>[key]);"
      break;
  ...
  m = ("/" + m + "/")
    .replace(/TODO!/, "This feature (@) is not yet ready, but is planned for future releases./Please note that the documentation for '@' is not final, and may change or be removed.//")
    .replace(/\//g, "\n")
    .replace(/~/g, "\t")
    .replace(/\\/g, "/")
    .replace(/\$-/g, "synq-")
    .replace(/\$/g, "SynQ")
    .replace(/(?:use_?)?(\w+)#/g, "use_$1_synq_token")
    .replace(/%/g, "element")
    .replace(/@/g, item)
    .replace(/(https?|synq)\:/g, "$1://")
